...
public:
    vector<int> decrypt(vector<int>& code, int k) {
        ...
        // 本題思考方式，可以透過時鐘來思考！(也可以稱為數學思維)
        // 利用週期性的方式（正向、逆向）解決
        // 被除數使用正數方式決定！
        // K != 0
        for(int i = 0; i < lengthCode; i++) {
            // k > 0, we need to calculate from the next number (正向)
            if( k > 0) {
                for (int j = 1; j <= k; j++) {
                    result[i] += code[(i + j) % lengthCode];
                }
            }

            // k < 0, we need to calculate from the previous number (負向)
            if(k < 0) {
                for(int j = 1; j <= abs(k); j++) {
                    result[i] += code[(i - j + lengthCode) % lengthCode];
                }
            }
        }
        return result;
