...
[Thought]
    1. There are two solutions. 
        ...
        - Using "Greedy"
            - Reference 03
            - We have to find out which small part could be optimized.
[Skill]
...
class Solution {
    int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
        // Record Total Usage
        int total = 0;
        // Set a Pointer to separate the interval
        int pointIndex = 0;
        // Record the accumulated sum
        int accumuSum = 0;

        // Using for loop to calculate the results
        for(int i = 0; i < gas.size(); i++) {
            // Record total Usage
            total += gas[i] - cost[i];
            // Record the accumulated value
            accumuSum += gas[i] - cost[i];
            if(accumuSum < 0) {
                pointIndex = i + 1;
                accumuSum = 0;
            }
