...
-109 <= nums[i] <= 109
    ...
    [Thought]:
    1. Using Sorted Array to get the longest sequences. (Brute Better Approach)
        - The method is optimized approach for Method 1.
    2. The time complexity is O(NlogN).
        - The time complexity of sort method in C++ or Python is O(NlogN).
        - O(NlogN) is bigger than O(N).
